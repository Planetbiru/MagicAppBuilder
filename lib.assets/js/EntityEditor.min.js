...
`}),a+="</select>"}setDefaultLength(e,t){let a=e.value;void 0!==this.defaultLength[a]&&(document.querySelector(t).value=this.defaultLength[a])}preference(){let e=this;e.showSettingDialog(`
        ...
        `;let p=document.createElement("option");p.value="",p.textContent="Select Column",h.classList.add("button-area"),r.appendChild(h),h.querySelector("select").appendChild(p),i.style.display="block",e.columns.forEach(e=>{let t=document.createElement("th");t.classList.add("entity-column"),t.textContent=e.name,t.dataset.name=e.name,d.appendChild(t);let a=document.createElement("option");a.value=e.name,a.textContent=e.name,h.querySelector("select").appendChild(a)}),c.appendChild(d),n.forEach((t,a)=>{let i=document.createElement("tr"),l=document.createElement("td");l.classList.add("td-remover");let r=document.createElement("a");r.className="delete-row",r.href="javascript:",r.textContent="❌",l.appendChild(r),i.appendChild(l),e.columns.forEach((e,l)=>{let r=this.createEntityDataCell(a,l,e,t[e.name]??"");i.appendChild(r)}),m.appendChild(i),r.addEventListener("click",function(e){e.preventDefault(),m.removeChild(i)})})}createEntityDataCell(e,t,a,i="",l=!1,r=null){let n=document.createElement("td");n.classList.add("entity-column");let s=document.createElement("input");if(s.type="text",s.classList.add("entity-data-cell"),s.name=`cell-${e}-${t}`,s.dataset.row=e,s.dataset.col=a.name,s.dataset.type=a.type,s.value=i??"",s.setAttribute("autocomplete","off"),s.style.width="100%",s.style.boxSizing="border-box",l){let o=document.querySelector(".suggestions");s.setAttribute("list",r),s.addEventListener("input",()=>{let e=s.value,t=document.querySelectorAll("#"+r+" option"),a=!1;for(let i of t)if(i.value===e){o.textContent=i.dataset.info||"",a=!0;break}a||(o.textContent="")})}return n.appendChild(s),n}exportData(){let e=this.entities[this.currentEntityIndex],t=[],a=[];document.querySelector(".data-preview-table").querySelector("thead").querySelectorAll("th.entity-column").forEach(e=>{t.push(e.dataset.name)});let i=document.querySelector(".data-preview-table").querySelector("tbody").querySelectorAll("tr");i&&i.forEach(e=>{let t={};e.querySelectorAll("td.entity-column").forEach(e=>{let a=e.querySelector("input");a&&a.name&&(t[a.dataset.col]=a.value)}),a.push(t)});let l=[];l.push(t.join(",")),a.forEach(e=>{let a=t.map(t=>`"${(e[t]||"").replace(/"/g,'""')}"`);l.push(a.join(","))});let r=l.join("\n"),n=new Blob([r],{type:"text/csv"}),s=URL.createObjectURL(n),o=document.createElement("a");o.href=s,o.download=`${e.name}.csv`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(s)}clearData(){this.showConfirmationDialog("Are you sure you want to delete all data?","Clear Entity Data Confirmation","Yes","No",e=>{e&&(document.querySelector(".data-preview-table tbody").innerHTML="")})}isAutocompleteListEmpty(){return"true"==document.querySelector(".autocomplete-list-container").dataset.empty}addAutocompleteList(e){if(this.isAutocompleteListEmpty()){let t=document.querySelector(".autocomplete-list-container");t.dataset.empty="false",t.innerHTML="",e.columns.forEach(e=>{if(e.name.length>3&&e.name.endsWith("_id")){let a=e.name.substring(0,e.name.length-3),i=this.getEntityByName(a);if(i&&i.data.length>0){let l=document.createElement("datalist");l.setAttribute("id",`list${a}`),i.data.forEach(t=>{let a=document.createElement("option"),i=t[e.name],r=t.name||t.nama||t.description||"";void 0!==i&&(a.value=t[e.name],a.dataset.info=r,l.appendChild(a))}),t.appendChild(l)}}})}}addData(e=!1){let t=document.querySelector("#entityDataEditorModal"),a=parseInt(t.dataset.index),i=this.entities[a];e&&this.addAutocompleteList(i);let l=t.querySelector(".data-preview-table tbody");if(!l)return console.warn("Table body for entity data editor not found."),null;let r=l.rows.length,n=document.createElement("tr"),s=document.createElement("td");s.classList.add("td-remover");let o=document.createElement("a");return o.className="delete-row",o.href="javascript:void(0);",o.textContent="❌",s.appendChild(o),n.appendChild(s),i.columns.forEach((t,a)=>{let i=null;e&&t.name.length>3&&t.name.endsWith("_id")&&(i=`list${t.name.substring(0,t.name.length-3)}`);let l=this.createEntityDataCell(r,a,t,"",e,i);n.appendChild(l)}),o.addEventListener("click",function(e){e.preventDefault(),n.remove()}),l.appendChild(n),n}saveData(){let e=document.querySelector("#entityDataEditorModal"),t=parseInt(e.dataset.index),a=this.entities[t],i=e.querySelectorAll(".data-preview-table input"),l={};i.forEach(e=>{let t=e.dataset.row,a=e.dataset.col;l[t]||(l[t]={}),l[t][a]=e.value});let r=Object.keys(l).sort((e,t)=>parseInt(e)-parseInt(t)).map(e=>l[e]);a.setData(r);let{applicationId:n,databaseName:s,databaseSchema:o,databaseType:c}=getMetaValues();sendEntityToServer(n,c,s,o,this.entities),this.exportToSQL(),e.style.display="none"}async exportHTMLDocument(e,t=!1){let a=[],i=[];e.forEach(e=>{e.entities&&e.entities.forEach(e=>{e&&!a.includes(e)&&a.push(e)});let t=document.querySelector(`#${e.id}`);if(t){t.dataset.name=e.name;let l=t.cloneNode(!0);i.push(l)}});let l=document.createElement("div");for(let r of(l.classList.add("export-container"),e)){let n=document.createElement("div");n.className="diagram-section";let s=i.find(e=>e.dataset.name===r.name);if(s){let o=document.createElement("div");o.className="svg-wrapper";let c=document.createElement("h3");if(c.textContent=`Diagram: ${s.dataset.name}`,o.appendChild(c),t)try{let d=s.querySelector("svg"),u=await convertSvgToPng(d),m=document.createElement("img");m.src=u,o.appendChild(m)}catch(h){console.error("❌ Failed to convert SVG to PNG",h),o.appendChild(s)}else o.appendChild(s);n.appendChild(o)}r.entities&&r.entities.forEach(e=>{let t=this.getEntityByName(e);if(t){let a=document.createElement("div");a.className="table-wrapper";let i=document.createElement("h3");i.textContent=`Entity: ${e}`,a.appendChild(i),t.description?.trim()&&t.description.trim().split(/\r?\n/).map(e=>e.trim()).filter(e=>""!==e).forEach(e=>{let t=document.createElement("p");t.textContent=e,a.appendChild(t)});let l=this.createHtmlEntity(t);a.appendChild(l),n.appendChild(a)}}),l.appendChild(n)}let p=document.createElement("div");p.classList.add("table-wrapper");let y=document.createElement("h3");y.textContent="Table: Entities Ordered by Dependency Depth",p.appendChild(y);let g=this.sortEntitiesByDepth(this.calculateEntityDepth(this.entities)),f=document.createElement("table"),E=document.createElement("thead"),b=document.createElement("tbody"),v=document.createElement("tr");f.appendChild(E),f.appendChild(b),E.appendChild(v);let S=document.createElement("th");S.classList.add("column-number"),S.textContent="#",v.appendChild(S);let D=document.createElement("th");D.textContent="Entity Name",v.appendChild(D);let w=document.createElement("th");w.textContent="Dependency Depth",v.appendChild(w),g.forEach((e,t)=>{let a=e.depth-1;a<0&&(a=0);let i=document.createElement("tr"),l=document.createElement("td");l.classList.add("column-number"),l.textContent=`${t+1}`,i.appendChild(l);let r=document.createElement("td");r.textContent=e.name,i.appendChild(r);let n=document.createElement("td");n.textContent=a,i.appendChild(n),b.appendChild(i)}),p.appendChild(f),l.appendChild(p);let C=new Date,T=C.toLocaleString(),L=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Exported Diagrams</title>
            <style>
