...
<td><input type="text" class="column-length" value="${l}" max="${m}" placeholder="Length" style="display: ${this.withLengthTypes.includes(o)?"inline-block":"none"};"></td>
...
<td class="column-ai"><input type="checkbox" class="column-autoIncrement" ${e.autoIncrement?"checked":""} ${c}></td>
        ...
        `,i.appendChild(n),this.initDraggableRow(n),t&&n.querySelector(".column-name").select()}foreignKeyEdit(e){let t=e.closest("tr"),a=t.qs(".column-name").value,i=qs(this.selector+" .entity-name").value;a&&this.showForeignKeyEditor(i,a,t)}showForeignKeyEditor(e,t,a){let i=this,n=qs("#foreignKeyModal"),l=`fk_${e}_${t}`,r=this.foreignKeyDraft[e]?.[t]||null;if(!r){let s=this.getEntityByName(e);r=this.getForeignKeyByColumnName(s.foreignKeys,t)}n.querySelector(".reference_table_selector").innerHTML="";let o="",c="",d="";if(this.entities.forEach(a=>{if(a.name!==e){let i=document.createElement("option");i.value=a.name,i.textContent=a.name,n.querySelector(".reference_table_selector").appendChild(i),(r?.referencedTable==a.name||`${a.name}_id`==t)&&(i.selected=!0,d=a.name)}}),r?(o=r.referencedTable,c=r.referencedColumn,l=r.name):(c=t,o=d),n.querySelector(".entity_name_selector").value=e,n.querySelector(".column_name_selector").value=t,n.querySelector(".foreign_key_name_selector").value=t,n.querySelector(".reference_table_selector").addEventListener("change",function(){let e=this.value,t=editor.getEntityByName(e),a=n.querySelector(".reference_column_selector");a.innerHTML="",t&&t.columns.forEach(e=>{let t=document.createElement("option");t.value=e.name,t.textContent=e.name,a.appendChild(t)})}),o){let m=editor.getEntityByName(o),h=n.querySelector(".reference_column_selector");h.innerHTML="",m&&m.columns.forEach(e=>{let t=document.createElement("option");t.value=e.name,t.textContent=e.name,e.name==c&&(t.selected=!0),h.appendChild(t)})}let u=qs(".save-foreign-key"),p=qs(".cancel-foreign-key"),y=qs(".delete-foreign-key"),g=()=>{let l=n.querySelector(".reference_table_selector").value,r=n.querySelector(".reference_column_selector").value,s=n.querySelector(".on_update_action_selector").value,o=n.querySelector(".on_delete_action_selector").value,c=n.querySelector(".foreign_key_name_selector").value;i.foreignKeyDraft[e]=i.foreignKeyDraft[e]||{},i.foreignKeyDraft[e][t]={name:c,columnName:t,referencedTable:l,referencedColumn:r,onUpdate:s,onDelete:o},a.classList.add("has-foreign-key"),u.removeEventListener("click",g),p.removeEventListener("click",f),n.style.display="none",this.exportToSQL(this.getSelectedDialect(),this.isGenerateForeignKey())},f=()=>{u.removeEventListener("click",g),p.removeEventListener("click",f),n.style.display="none"},E=()=>{i.foreignKeyDraft[e]?.[t]&&(delete i.foreignKeyDraft[e][t],a.classList.remove("has-foreign-key")),n.style.display="none"};n.style.display="block",u.addEventListener("click",g),p.addEventListener("click",f),y.addEventListener("click",E)}excludeSuffixId(e){return e.length>3&&e.endsWith("_id")?e.substring(0,e.length-3):e}createInputText(e,t){return`<input type="text" class="form-control ${e}" value="${t}">`}createSelectOption(e,t,a,i=null){if(!Array.isArray(a))return console.warn("Options must be an array."),"";let n=e=>String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),l="";i&&(l=` onchange="${i}"`);let r=`<select${e?` class="form-control ${n(e)}"`:""}${l}>\r
...
`}),r+=`</select>\r
`}removeForeignKey(e){let t=e.closest("tr");return t.parentNode.removeChild(t),!1}addForeignKey(e){let t=e.closest(".modal").querySelector("tbody"),a=selectedElement.dataset.entity,i=[],n=[],l="";t.querySelectorAll(".column-name")?.forEach(e=>{i.push(e.value)}),this.getEntityByName(a)?.columns.forEach(e=>{n.push(e.name),i.includes(e.name)||e.primaryKey||""!=l||(l=e.name)});let r={name:`fk_${a}_${l}`,columnName:l,referencedTable:null,referencedColumn:null,onUpdate:"NO ACTION",onDelete:"NO ACTION",draft:!0};t.appendChild(this.createForeignKeyRow(r))}updateForeignKeyColumn(e){let t=e.closest("tr"),a=e.value,i=selectedElement.dataset.entity,n=`${i}_${a}`;t.querySelector(".foreign-key-name").value=n}updateForeignKeyReferencedColumn(e){let t=e.closest("tr"),a=e.value,i=t.querySelector(".referenced-column"),n=i.value;i.dataset.currentValue=n;let l=this.getEntityByName(a),r=t.querySelector(".referenced-column");r.innerHTML="",l&&l.columns.forEach(e=>{r.appendChild(this.createOption(e))})}createOption(e){let t=document.createElement("option");return t.value=e.name,t.textContent=e.name,e.primaryKey&&t.setAttribute("selected","selected"),t}createForeignKeyRow(e){let t=[],a=[],i=[];if(this.entities.forEach(e=>{t.push(e.name)}),this.getEntityByName(selectedElement.dataset.entity)?.columns.forEach(e=>{a.push(e.name)}),!e.referencedTable){let n=this.excludeSuffixId(e.columnName);n&&t.includes(n)&&(e.referencedTable=n)}this.getEntityByName(e.referencedTable)?.columns.forEach(t=>{i.push(t.name),!e.referencedColumn&&t.primaryKey&&(e.referencedColumn=t.name)});let l=["NO ACTION","RESTRICT","CASCADE","SET NULL"],r=document.createElement("tr");return r.innerHTML=`<td><a href="javascript:" onclick="return editor.removeForeignKey(this)">❌</a></td>
        <td>${this.createSelectOption("column-name",e.columnName,a,"editor.updateForeignKeyColumn(this)")}</td>
        <td>${this.createInputText("foreign-key-name",e.name)}</td>
        <td>${this.createSelectOption("referenced-table",e.referencedTable,t,"editor.updateForeignKeyReferencedColumn(this)")}</td>
        <td>${this.createSelectOption("referenced-column",e.referencedColumn,i)}</td>
        <td>${this.createSelectOption("on-update",e.onUpdate,l)}</td>
        <td>${this.createSelectOption("on-delete",e.onDelete,l)}</td>`,e.draft?r.classList.add("pk-draft"):r.classList.add("pk-saved"),r}editForeignKeyContextMenu(e){let t=qs("#foreignKeyBulkModal"),a=[];this.entities.forEach(e=>{a.push(e.name)});let i=qs(".save-foreign-key-bulk"),n=qs(".cancel-foreign-key-bulk"),l=this.getEntityByName(selectedElement.dataset.entity),r=l.foreignKeys||[];if(Array.isArray(r)){let s={};r.forEach(e=>s[e.columnName]=e),this.foreignKeyDraft[l.name]=JSON.parse(JSON.stringify(s))}else this.foreignKeyDraft[l.name]=JSON.parse(JSON.stringify(r));l.columns.forEach(e=>{let t=this.excludeSuffixId(e.name);t!=l.name&&!this.foreignKeyDraft[l.name][e.name]&&t&&a.includes(t)&&e.name.endsWith("_id")&&(this.foreignKeyDraft[l.name][e.name]={name:`fk_${l.name}_${e.name}`,columnName:e.name,referencedTable:t,referencedColumn:e.name,onUpdate:"NO ACTION",onDelete:"NO ACTION",draft:!0})});let o=t.querySelector(".foreign-key-editor-table tbody");t.querySelector(".entity-name").innerHTML=`Entity Name : <strong>${l.name}</strong>`,o.innerHTML="",Object.entries(this.foreignKeyDraft[l.name]).forEach(([e,t])=>{o.appendChild(this.createForeignKeyRow(t))}),t.style.display="block";let c=()=>{let e={},a=!0,r=o.querySelectorAll("tr");if(r&&r.forEach(t=>{let i=t.querySelector(".foreign-key-name").value,n=t.querySelector(".column-name").value,l=t.querySelector(".referenced-table").value,r=t.querySelector(".referenced-column").value,s=t.querySelector(".on-update").value,o=t.querySelector(".on-delete").value;if(""==i.trim()||""==n.trim()||""==l.trim()||""==r.trim()||""==s.trim()||""==o.trim()){let c=`Invalid foreign key definition for ${n}.`;this.showAlertDialog(c,"Invalid Foreign Key","Close"),a=!1;return}e[n]={name:i,columnName:n,referencedTable:l,referencedColumn:r,onUpdate:s,onDelete:o}}),a){l.foreignKeys=Object.values(e),this.callbackSaveEntity(this.entities),this.renderEntities(),this.updateDiagram();let s=qs(".tabs-link-container li.diagram-tab.active");s&&this.selectDiagram(s),this.cancelEdit(),i.removeEventListener("click",c),n.removeEventListener("click",d),t.style.display="none",this.exportToSQL(this.getSelectedDialect(),this.isGenerateForeignKey())}},d=()=>{i.removeEventListener("click",c),n.removeEventListener("click",d),t.style.display="none"};i.addEventListener("click",c),n.addEventListener("click",d),hideContextMenu()}initDraggableRow(e){let t=this;(void 0===t.tbody||null===t.tbody)&&(t.tbody=e.closest("tbody")),"true"!==e.dataset.hasDragEvent&&(e.dataset.hasDragEvent="true",e.addEventListener("mousedown",function(t){let a=t.target.tagName,i=["INPUT","TEXTAREA","SELECT"].includes(a);e.setAttribute("draggable",i?"false":"true")}),e.addEventListener("dragstart",function(e){t.dragSrcRow=this,this.classList.add("dragging"),e.dataTransfer.effectAllowed="move"}),e.addEventListener("dragend",function(){this.classList.remove("dragging")}),e.addEventListener("dragover",function(e){e.preventDefault(),e.dataTransfer.dropEffect="move",this.classList.add("over")}),e.addEventListener("dragleave",function(){this.classList.remove("over")}),e.addEventListener("drop",function(e){e.preventDefault();let a=this.closest("tbody");if(t.dragSrcRow!==this){let i=Array.from(a.children),n=i.indexOf(t.dragSrcRow),l=i.indexOf(this);n<l?a.insertBefore(t.dragSrcRow,this.nextSibling):a.insertBefore(t.dragSrcRow,this),"function"==typeof t.updateRowNumbers&&t.updateRowNumbers()}this.classList.remove("over")}))}addColumn(e=!1){let t=this.selector+" .entity-container .entity-name",a=qs(t),i=a.value;if("create"==this.operation&&this.isEntityExists(i)){this.showConfirmationDialog(`Entity '${i}' already exists.`,"Duplicate Entity Detected","Rename","Close",e=>{e&&(a.value=`${i}_new`,a.select())});return}let n=qsa(this.selector+" .entity-container .column-name").length,l=0===n?`${i}_id`:`${i}_col${0===n?"":n+1}`,r=new Column(l,this.defaultDataType,this.defaultDataLength);r.nullable=!0,this.addColumnToTable(r,e,!0);let s=qs(this.selector+" .entity-container .table-container");s.scrollTop=s.scrollHeight}isEntityExists(e){for(let t of this.entities)if(t.name===e)return!0;return!1}hasPrimaryKey(){let e=qsa(this.selector+" #table-entity-editor .column-primary-key");for(let t of e)if(t.checked)return!0;return!1}saveEntity(){let e=this.selector+" .entity-container .entity-name",t=qs(e),a=t.value;if(!this.hasPrimaryKey()){this.showAlertDialog("A primary key is required before saving the entity.","Primary Key Required","OK");return}let i=this.checkDuplicatedColumn();if(i){this.showAlertDialog(`Duplicate column name: '${i.name}'`,"Duplicated Column Name","OK",function(){i.element.select()});return}if("create"==this.operation&&this.isEntityExists(a)){this.showConfirmationDialog(`Entity '${a}' already exists.`,"Duplicate Entity Detected","Rename","Close",e=>{e&&(t.value=`${a}_new`,t.select())});return}this.doSaveEntity();let n=qs(".tabs-link-container li.diagram-tab.active");n&&this.selectDiagram(n)}checkDuplicatedColumn(){let e=[],t=qsa(this.selector+" #table-entity-editor .column-name");for(let a=0;a<t.length;a++){let i=t[a].value;if(e.includes(i))return{index:a,name:i,element:t[a]};e.push(i)}return null}doSaveEntity(){let e=qs(this.selector+" .entity-name").value,t=[],a=[],i=qsa(this.selector+" #table-entity-editor .column-name"),n=qsa(this.selector+" #table-entity-editor .column-type"),l=qsa(this.selector+" #table-entity-editor .column-nullable"),r=qsa(this.selector+" #table-entity-editor .column-default"),s=qsa(this.selector+" #table-entity-editor .column-primary-key"),o=qsa(this.selector+" #table-entity-editor .column-autoIncrement"),c=qsa(this.selector+" #table-entity-editor .column-length"),d=qsa(this.selector+" #table-entity-editor .column-enum"),m=qsa(this.selector+" #table-entity-editor .column-description"),h=[];for(let u=0;u<i.length;u++){let p=new Column(i[u].value,n[u].value,c[u].value||null,l[u].checked,r[u].value||null,s[u].checked,o[u].checked,d[u].value||null,m[u].value||null);i[u].dataset.originalName&&i[u].dataset.originalName!=i[u].value&&h.push({original:i[u].dataset.originalName,new:i[u].value}),t.push(p),a.push(new Column(i[u].dataset.originalName,n[u].value,c[u].value||null,l[u].checked,r[u].value||null,s[u].checked,o[u].checked,d[u].value||null,m[u].value||null))}if(h.forEach(e=>{this.entities[this.currentEntityIndex]&&this.entities[this.currentEntityIndex].data&&this.entities[this.currentEntityIndex]&&this.entities[this.currentEntityIndex].data.forEach(t=>{t.hasOwnProperty(e.original)&&(t[e.new]=t[e.original],delete t[e.original])})}),this.currentEntityIndex>=0){this.entities[this.currentEntityIndex].name=e,this.entities[this.currentEntityIndex].index=this.currentEntityIndex,this.entities[this.currentEntityIndex].columns=t,this.entities[this.currentEntityIndex].modificationDate=new Date().getTime(),this.entities[this.currentEntityIndex].modifier="{{userName}}";let y=this.foreignKeyDraft[e]?Object.values(this.foreignKeyDraft[e]):null;this.entities[this.currentEntityIndex].foreignKeys=y||this.entities[this.currentEntityIndex].foreignKeys||[],this.foreignKeyDraft[e]=null}else{let g=new Entity(e,this.entities.length);t.forEach(e=>g.addColumn(e));let f=this.snakeizeData(this.currentEntityData,a);h.forEach(e=>{f.forEach(t=>{t.hasOwnProperty(e.original)&&(t[e.new]=t[e.original],delete t[e.original])})}),g.setData(f),g.modificationDate=new Date().getTime(),g.creationDate=g.modificationDate,g.creator="{{userName}}",g.modifier="{{userName}}",g.foreignKeys=this.foreignKeyDraft[e]?Object.values(this.foreignKeyDraft[e]):[],this.foreignKeyDraft[e]=null,this.entities.push(g)}this.renderEntities(),this.updateDiagram(),this.cancelEdit(),this.exportToSQL(this.getSelectedDialect(),this.isGenerateForeignKey()),"function"==typeof this.callbackSaveEntity&&this.callbackSaveEntity(this.entities)}updateDiagram(){let e=this;qs(".diagram-container").querySelectorAll(".diagram-entity").forEach(t=>{let a=t.getAttribute("id"),i=t.closest(".left-panel").offsetWidth,n=(t.dataset.entities||"").split(","),l=[];n.forEach(t=>{let a=e.getEntityByName(t);null!=a&&l.push(a)}),(isNaN(i)||i<240)&&(i=240),diagramRenderer[a].createERD({entities:l},i-240,qs("#draw-auto-relationship").checked,qs("#draw-fk-relationship").checked);let r=t.querySelector("svg");e.removeDiagramEventListener(r),e.addDiagramEventListener(r)})}saveDiagram(){let e=this.getCheckedEntities(),t=[],a=0;Object.entries(e).forEach(([e,i],n)=>{let l=qs(`.tabs-link-container [data-id="${e}"] input`).value;t.push({id:e,name:l,sortOrder:a,entities:i}),a++}),this.diagrams=t,"function"==typeof this.callbackSaveDiagram&&this.callbackSaveDiagram(t)}showEditorTemplate(){this.currentEntityIndex=-2,qs(this.selector+" .template-columns-table-body").innerHTML="",this.template.columns.forEach(e=>this.addColumnToTemplate(e)),qs(this.selector+" .button-container").style.display="none",qs(this.selector+" .entity-container").style.display="none",qs(this.selector+" .template-container").style.display="block",qs(this.selector+" .editor-form").style.display="block"}addColumnTemplate(e){this.addColumnToTemplate({name:"",type:"VARCHAR",length:"50",nullable:!0,default:null,values:""},e);let t=qs(this.selector+" .template-container .table-container");t.scrollTop=t.scrollHeight}addColumnToTemplate(e,t){let a=qs(this.selector+" .template-columns-table-body"),i=document.createElement("tr"),n=null==e.length?"":e.length.replace(/\D/g,""),l=null==e.default?"":e.default,r=e.type.split("(")[0].trim(),s=e.description?e.description:"";i.innerHTML=`
<td class="drag-handle"></td>
<td class="column-action">
    <button onclick="editor.removeColumn(this)" class="icon-emoji icon-delete"></button>
    <button onclick="editor.moveUp(this)" class="icon-emoji icon-move-up"></button>
...
<td>[Custom Entities]</td>
            `,r.appendChild(d)}let m=0;if(this.entities.forEach((t,a)=>{!e.systemEntities.includes(t.name.toLowerCase())&&(e.appendBuildTableRow(r,t,m,"custom"),m++)}),this.entities.some(t=>e.systemEntities.includes(t.name.toLowerCase()))){let h=document.createElement("tr");h.classList.add("group-header"),h.innerHTML=`
<td>
...
<td>[Systen Entities]</td>
                `,r.appendChild(h)}this.entities.forEach((t,a)=>{e.systemEntities.includes(t.name.toLowerCase())&&(e.appendBuildTableRow(r,t,m,"system"),m++)}),this.entities.forEach((e,t)=>{let a=document.createElement("li");a.innerHTML=`
...
<tr>
    ...
    <th>PK Value</th>
        ...
        `,c.appendChild(d);let m=document.createElement("tbody");e.columns.forEach(e=>{let t=document.createElement("tr"),a=e.type||"";null!=e.length&&""!==e.length&&(a+=`(${e.length})`),t.dataset.col=e.name;let n="",l=-1!=e.type.toLowerCase().indexOf("text")?" checked":"";e.primaryKey||e.name.endsWith("_id")||(n=`<input type="checkbox" class="textarea-graphql" data-col="${e.name}"${l}>`),t.innerHTML=`
...
`,m.appendChild(t)}),c.appendChild(m),n.appendChild(c),t.appendChild(n)}getFilterType(e,t){let a="";return e.primaryKey||(e.name==t?a="text-CONTAINS":e.name.endsWith("_id")&&(a="select-EQUALS")),`
