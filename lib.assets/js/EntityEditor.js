...
<td class="column-ai"><input type="checkbox" class="column-autoIncrement" ${column.autoIncrement ? 'checked' : ''} ${aiDisabled}></td>
    ...
    showForeignKeyEditor(entityName, columnName, row) {
        ...
        if(!this.foreignKeyDraft[entityName] || !Array.isArray(this.foreignKeyDraft[entityName]))
        {
            this.foreignKeyDraft[entityName] = entity.foreignKeys;
        }
        let fk = this.getForeignKeyByColumnName(entity.foreignKeys, columnName) || null;
        ...
        const onSave = () => {
            ...
            _this.foreignKeyDraft[entityName].push({
                name: fkName,
        ...
        const onDelete = () => {
            let fks = [];
            _this.foreignKeyDraft[entityName]?.forEach((fk) => {
                if(fk.columnName != columnName) {
                    fks.push(fk);
                }
            });
            row.classList.remove('has-foreign-key');
            _this.foreignKeyDraft[entityName] = fks;
            // Hide the modal
            modal.style.display = 'none';
        };
    ...
    addForeignKey(elem)
    {
        ...
        tbody.appendChild(this.createForeignKeyRow(fk, entity));
    }
...
<a
                ...
                entities.forEach(entity => {
                    if(selected.includes(entity.name))
                    {
                        ...
                        newEntity.columns.forEach(col => {
                            ...
                            if(col.primaryKey)
                            {
                                let pkValue = entitySelector.querySelector(`select.pk-value-graphql[data-col="${col.name}"]`);
                                if(pkValue)
                                {
                                    col.primaryKeyValue = pkValue.value;
                                }
                                else
                                {
                                    col.primaryKeyValue = 'autogenerated'; // User can not provide primary key value
                                }
                            }
    ...
    getPrimaryKeyValue(col)
    {
        ...
        {
            return `
            <select class="form-control pk-value-graphql" data-col="${col.name}">
...
<td><button class="btn btn-tn" onclick="editor.gqlAddRow(this)">Add</button></td>
    ...
    async exportHTMLDocument(diagramToExport, usePng = false) {
        ...
        th0.textContent = '#';
        tr21.appendChild(th0);
        let th1 = document.createElement('th');
        th1.textContent = 'Entity Name';
        tr21.appendChild(th1);
