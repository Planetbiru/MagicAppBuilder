...
- **Primary key generation options** (`autogenerated`, `manual-insert`, `manual-all`) for GraphQL applications.
...
- **GraphQL API documentation generation** in both Markdown (`MANUAL.md`) and interactive HTML (`manual.html`) formats.
...
- **Right-to-Left (RTL) language support** for generated GraphQL applications.
...
- **Customizable application URL**, `<textarea>` support, and the ability to add custom HTML elements.
