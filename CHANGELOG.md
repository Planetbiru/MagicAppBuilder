...
* When importing Excel files (`.xlsx` or `.xls`) that contain more than one sheet, users will be prompted to select the sheet they want to import before the entity structure is generated.
...
### Enhancement: Truncated Entity Name in SVG View

* Long entity names in the Entity Editor are now automatically truncated to prevent visual overflow.
* The display uses SVG `<foreignObject>` with CSS `text-overflow: ellipsis` to ensure that long names do not overlap adjacent icons or elements.

#### Benefits:

* Maintains visual clarity and alignment of action icons (edit, delete, move).
* Provides a cleaner and more professional diagram view, especially for autogenerated or lengthy names.
...
Improves developer ergonomics when working with deeply nested data structures.
...
Provides more flexibility while maintaining precision down to the second.
