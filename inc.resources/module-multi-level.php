...
$userPermission = new AppUserPermissionImpl($appConfig, $database, $appUserRole, $currentModule, $currentUser, true);
...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$module->setParentModuleId($inputPost->getParentModuleId(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS, false, false, true));
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	$updater->with()
		...
		->setParentModuleId($inputPost->getParentModuleId(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS, false, false, true))
		...
		<form name="createform" id="createform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getParentModule();?></td>
						<td>
...
require_once $appInclude->mainAppHeader(__DIR__);
	$specification = PicoSpecification::getInstanceOf(Field::of()->moduleId, $inputGet->getModuleId(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS));
	$specification->addAnd($dataFilter);
	$module = new AppModuleMultiLevelImpl(null, $database);
	try{
		$module->findOne($specification);
		if($module->issetModuleId())
		{
		...
		<form name="detailform" id="detailform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getParentModule();?></td>
						<td><?php echo $module->issetParentModule() ? $module->getParentModule()->getName() : "";?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getUrl();?></td>
...
$originalParentModuleId = $inputGet->getParentModuleId();

if($inputGet->getParentModuleId() == '__root_menu__')
{
	// manipulate
	$inputGet->setParentModuleId('');
}


$specMap = array(
	"parentModuleId" => PicoSpecification::filter("parentModuleId", "fulltext"),
...
if($originalParentModuleId == '__root_menu__')
{
	$specification->addAnd(
		PicoSpecification::getInstance()
			->addOr(PicoPredicate::getInstance()->equals(Field::of()->parentModuleId, null))
			->addOr(PicoPredicate::getInstance()->equals(Field::of()->parentModuleId, ''))
	);	
}
$specification->addAnd($dataFilter);
...
$dataLoader = new AppModuleMultiLevelImpl(null, $database);


if($originalParentModuleId == '')
{
		...
		<div class="filter-section">
			<form action="" method="get" class="filter-form">
				<span class="filter-group">
					<span class="filter-label"><?php echo $appEntityLanguage->getParentModule();?></span>
					<span class="filter-control">
							<select class="form-control" name="parent_module_id" id="parent_module_id" onchange="this.form.submit()">
					...
					<table class="table table-row table-sort-by-column">
						<thead>
							<tr>
								...
								<td data-col-name="parent_module_id" class="order-controll"><a href="#"><?php echo $appEntityLanguage->getParentModule();?></a></td>
						...
						<tbody class="data-table-manual-sort" data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-primary-key="<?php echo $module->getModuleId();?>" data-sort-order="<?php echo $module->getSortOrder();?>" data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>" data-active="<?php echo $module->optionActive('true', 'false');?>">
								...
								<td data-col-name="parent_module_id"><?php echo $module->issetParentModule() ? $module->getParentModule()->getName() : "";?></td>
