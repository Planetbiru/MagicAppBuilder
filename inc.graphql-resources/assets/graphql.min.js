class GraphQLClientApp{constructor(t={}){let e={configUrl:"frontend-config.php",apiUrl:"graphql.php",loginUrl:"login.php",logoutUrl:"logout.php",entityLanguageUrl:"entity-language.php",i18nUrl:"language.php",languageConfigUrl:"available-language.php",customRenderers:{},defaultActiveField:"active",defaultDisplayField:"name",languageId:null,defaultLanguage:"en",pages:{}};Object.assign(this,e,t),this.supportedLanguages={},this.uiTranslations={},this.entityLanguagePack={},this.config=null,this.currentEntity=null,this.state={page:1,limit:10,filters:{},orderBy:{}},this.i18n={},this.dom={menu:document.getElementById("entity-menu"),title:document.getElementById("content-title"),body:document.getElementById("content-body"),modal:document.getElementById("form-modal"),modalTitle:document.getElementById("modal-title"),form:document.getElementById("entity-form"),closeModalBtn:document.querySelector(".close-button"),loginModal:document.getElementById("login-modal"),loginForm:document.getElementById("login-form"),loginCloseBtn:document.getElementById("login-close-button"),logoutBtn:document.querySelector(".logout-link"),logoutBtnDropdown:document.getElementById("logout-btn-dropdown"),sidebarToggle:document.getElementById("sidebar-toggle"),sidebar:document.getElementById("sidebar-nav"),mainContent:document.getElementById("main-content"),langMenu:document.getElementById("lang-menu"),filterContainer:document.getElementById("filter-container"),tableDataContainer:document.getElementById("table-data-container"),paginationContainer:document.getElementById("pagination-container"),loadingBar:document.getElementById("loading-bar"),themeToggle:document.getElementById("theme-toggle"),pageWrapper:document.getElementById("page-wrapper"),infoModal:document.getElementById("infoModal"),infoModalTitle:document.getElementById("infoModalTitle"),infoModalMessage:document.getElementById("infoModalMessage"),infoModalOk:document.getElementById("infoModalOk")},this.applicationTitle=document.querySelector('meta[name="title"]').getAttribute("content"),this.init()}initPage(){this.dom.sidebarToggle&&this.dom.sidebar&&this.dom.mainContent&&this.dom.sidebarToggle.addEventListener("click",()=>{let t=document.documentElement.classList.toggle("sidebar-collapsed");localStorage.setItem("sidebarCollapsed",t)}),this.dom.sidebar.classList.add("sidebar-animated"),document.querySelectorAll("[data-dropdown]").forEach(t=>{t.addEventListener("click",e=>{e.stopPropagation();let i=t.dataset.dropdown,a=document.getElementById(i);a&&(document.querySelectorAll(".dropdown-menu.active").forEach(t=>{t!==a&&t.classList.remove("active")}),a.classList.toggle("active"))})}),window.addEventListener("click",()=>{document.querySelectorAll(".dropdown-menu.active").forEach(t=>t.classList.remove("active"))}),this.dom.logoutBtnDropdown&&(this.dom.logoutBtnDropdown.onclick=t=>this.handleLogout(t)),this.dom.langMenu.addEventListener("click",t=>{if(t.target.matches("a[data-lang]")){t.preventDefault();let e=t.target.dataset.lang;this.changeLanguage(e)}}),this.dom.themeToggle.addEventListener("click",()=>this.toggleTheme()),window.addEventListener("storage",t=>{"theme"===t.key&&this.applyTheme(t.newValue)});let t=localStorage.getItem("theme")||(window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");this.applyTheme(t)}handleUnauthorized(){"block"!==this.dom.loginModal.style.display&&(this.dom.pageWrapper.style.display="none",document.getElementById("login-error").textContent=this.t("session_expired"),this.openLoginModal())}async init(){this.dom.loginCloseBtn.onclick=()=>this.closeLoginModal(),this.dom.loginForm.onsubmit=t=>this.handleLogin(t),this.dom.logoutBtn.onclick=t=>this.handleLogout(t);try{await this.initializeLanguage(),await this.loadI18n(),await this.loadConfig(),await this.loadLanguage(),this.applyI18n(),this.initPage(),window.onclick=t=>{t.target==this.dom.modal&&this.closeModal()},document.addEventListener("click",t=>{let e=t.target.closest('[data-dismiss="modal"]');if(e){let i=e.closest(".modal");i&&(i.id===this.dom.modal.id?this.closeModal():i.id===this.dom.loginModal.id?this.closeLoginModal():(i.id,this.dom.infoModal.id,i.classList.remove("show")))}}),this.buildMenu(),window.addEventListener("popstate",()=>this.handleRouteChange()),this.handleRouteChange()}catch(t){console.error("Initialization Error:",t),this.dom.body.innerHTML=`<p style="color: red;">Error: ${t.message}</p>`}}_invokeRenderHook(t,e){let i=e.entity.name;return!!this.customRenderers[i]&&"function"==typeof this.customRenderers[i][t]&&(this.customRenderers[i][t](e),!0)}async initializeLanguage(){try{let t=await fetch(this.languageConfigUrl);if(!t.ok)throw Error(`Could not fetch ${this.languageConfigUrl}`);let e=await t.json();this.supportedLanguages=e.supported,this.defaultLanguage=e.default;let i=localStorage.getItem("userLanguage"),a=this.defaultLanguage;if(i&&this.supportedLanguages[i])a=i;else{let n=navigator.language.split("-")[0];this.supportedLanguages[n]&&(a=n)}this.languageId=a,localStorage.setItem("userLanguage",this.languageId),localStorage.setItem("languageId",this.languageId),document.documentElement.lang=this.languageId,this.populateLangMenu()}catch(s){console.error("Failed to initialize language configuration, falling back to default:",s),this.supportedLanguages={en:"English"},this.defaultLanguage="en",this.languageId=this.defaultLanguage,localStorage.setItem("userLanguage",this.languageId),localStorage.setItem("languageId",this.languageId),document.documentElement.lang=this.languageId,this.populateLangMenu()}}populateLangMenu(){for(let[t,e]of(this.dom.langMenu.innerHTML="",Object.entries(this.supportedLanguages))){let i=document.createElement("li"),a=document.createElement("a");a.href="#",a.dataset.lang=t,t===this.languageId&&a.classList.add("active"),a.textContent=e,i.appendChild(a),this.dom.langMenu.appendChild(i)}}async loadConfig(){let t=await fetch(this.configUrl);if(401===t.status)throw this.handleUnauthorized(),Error("Authentication required.");if(!t.ok)throw Error(`Failed to load config from ${this.configUrl}`);this.config=await t.json(),this.config.pagination&&this.config.pagination.pageSize&&(this.state.limit=this.config.pagination.pageSize)}async loadLanguage(){try{if(this.entityLanguageUrl){let t=`${this.entityLanguageUrl}?lang=${this.languageId}`,e=await fetch(t);if(401===e.status)throw this.handleUnauthorized(),Error("Authentication required.");if(!e.ok)throw Error(`Failed to load language from ${this.entityLanguageUrl}`);let i=await e.json();for(let a in this.entityLanguagePack[this.languageId]={},i.entities)this.entityLanguagePack[this.languageId][a]=i.entities[a].columns}}catch(n){console.warn(`Could not load entity language file for '${this.languageId}'. Falling back to generated labels.`,n)}}async loadI18n(){try{if(this.i18nUrl){let t=`${this.i18nUrl}?lang=${this.languageId}`,e=await fetch(t);if(401===e.status)throw this.handleUnauthorized(),Error("Authentication required.");if(!e.ok)throw Error(`Failed to load i18n from ${this.i18nUrl}`);this.i18n=await e.json()}}catch(i){console.warn("Could not load language file. Falling back to key-based labels.",i),this.i18n={}}}applyI18n(){document.querySelectorAll("[data-i18n]").forEach(t=>{let e=t.getAttribute("data-i18n");"INPUT"===t.tagName||"TEXTAREA"===t.tagName?t.placeholder=this.t(e):t.textContent=this.t(e)}),document.querySelectorAll("[data-i18n-title]").forEach(t=>{let e=t.getAttribute("data-i18n-title");t.title=this.t(e)})}t(t,...e){let i=this.i18n[t];if(i)return e.forEach((t,e)=>{let a=RegExp(`\\{${e}\\}`,"g");i=i.replace(a,t)}),i;let a=this._generateLabelFromKey(t);return[a,...e].join(" ")}changeLanguage(t){localStorage.setItem("userLanguage",t),localStorage.setItem("languageId",t),window.location.reload()}toggleTheme(){let t=document.documentElement.getAttribute("data-theme")||"light",e="dark"===t?"light":"dark";this.applyTheme(e),localStorage.setItem("theme",e)}applyTheme(t){document.documentElement.setAttribute("data-theme",t)}getEntityLabel(t,e){let i=this.snakeCase(t.name);if(!this.entityLanguagePack||!this.entityLanguagePack[this.languageId])return this.snakeCaseToTitleCase(e);{let a=this.entityLanguagePack[this.languageId][t.name];return(a||(a=this.entityLanguagePack[this.languageId][i]),a&&a[e])?a[e]:this.snakeCaseToTitleCase(e)}}getPreferredLanguages(){return navigator.languages&&navigator.languages.length?navigator.languages:navigator.language?[navigator.language]:["en"]}buildMenu(){this.config&&this.config.entities&&(this.dom.menu.innerHTML="",Object.values(this.config.entities).forEach((t,e)=>{let i=document.createElement("li"),a=document.createElement("a");a.href=`#${t.name}`,a.textContent=t.displayName,a.onclick=e=>{e.preventDefault(),this.navigateTo(t.name,{limit:this.state.limit})},i.appendChild(a),this.dom.menu.appendChild(i)}))}async gqlQuery(t,e={}){this.dom.loadingBar.style.display="block";try{let i=await fetch(this.apiUrl,{method:"POST",headers:{"Content-Type":"application/json","X-Requested-With":"xmlhttprequest","X-Language-Id":this.languageId,"Accept-Language":this.languageId,Accept:"application/json"},body:JSON.stringify({query:t,variables:e})});if(401===i.status)throw this.handleUnauthorized(),Error("Authentication required.");let a=await i.json();if(a.errors)throw console.error("GraphQL Errors:",a.errors),console.error(`Error: ${a.errors[0].message}`),Error(a.errors[0].message);return a.data}catch(n){throw n}finally{this.dom.loadingBar.style.display="none"}}navigateTo(t,e={}){let i=e.filters||{},a=e.orderBy||{},n=new URLSearchParams;n.set("page",e.page||1),n.set("limit",e.limit||10),Object.entries(i).forEach(([t,e])=>{e&&n.set(t,e)}),a.field&&(n.set("orderBy",a.field),n.set("orderDir",a.direction));let s=`${window.location.pathname}#${t}?${n.toString()}`;history.pushState({entityName:t,params:e},"",s),this.handleRouteChange()}navigateToDetail(t,e){let i=`${window.location.pathname}#${t}/detail/${e}`;history.pushState({entityName:t,id:e},"",i),this.handleRouteChange()}async handlePage(t){let e=this.pages[t];if(e)try{let i=this.t(e.title);if(this.dom.title.textContent=i,document.title=`${i} - ${this.applicationTitle}`,e.url){let[a,n]=e.url.split("?"),s={},l={},r=window.location.hash,[o,d]=r.split("?"),h=new URLSearchParams;if(n)for(let[c,u]of(s=new URLSearchParams(n)).entries());if(d)for(let[g,m]of(l=new URLSearchParams(d)).entries())h.set(g,m);e.url=`${a}?${h.toString()}`,this.dom.loadingBar.style.display="block";let y={method:e.method,headers:{"X-Requested-With":"xmlhttprequest","X-Language-Id":this.languageId,"Accept-Language":this.languageId,Accept:e.accept||"*"}};e.body&&(y.body=e.body);let p=await fetch(e.url,y);if(200!=p.status){"function"==typeof e.error&&e.error(p.status,p.statusText,this.dom.tableDataContainer,this.dom);return}let f;f=e?.accept&&-1!=e.accept?.indexOf("json")?await p.json():await p.text(),"function"==typeof e.success&&e.success(f,this.dom.tableDataContainer,this.dom)}else e.content&&"function"==typeof e.render&&e.render(e.content,this.dom.tableDataContainer,this.dom)}catch(b){throw b}finally{this.dom.loadingBar.style.display="none"}}async handleRouteChange(){let t=window.location.hash.substring(1);if(!t||"#"===t){this.renderDashboardView();return}let[e,i]=t.split("?"),a=e.split("/");if(this.pages[a[0]]){this.handlePage(a[0]);return}let n=a[0],s=a.length>1?a[1]:"list",l=a.length>2?a[2]:null,r=new URLSearchParams(i),o=this.config.entities[n];if(!o){this.dom.body.innerHTML=`<p>Page "${n}" not found.</p>`;return}this.currentEntity=o;let d=this.applicationTitle;document.title=`${o.displayName} - ${d}`;let h={};for(let[c,u]of r.entries())"page"!==c&&"limit"!==c&&(h[c]=u);let g={};r.has("orderBy")&&(g.field=r.get("orderBy"),g.direction=r.get("orderDir")||"ASC");let m=r.get("limit"),y=null!==m?parseInt(m,10):this.state.limit||10;if(this.state={page:parseInt(r.get("page"))||1,limit:y,filters:h,orderBy:g},this.config&&this.config.pagination){let{minPageSize:p,maxPageSize:f}=this.config.pagination;p&&this.state.limit<p&&(this.state.limit=p),f&&this.state.limit>f&&(this.state.limit=f)}document.querySelectorAll("#entity-menu a").forEach(t=>t.classList.toggle("active",t.getAttribute("href")===`#${n}`)),"detail"===s&&l?(this.dom.filterContainer.style.display="none",this.clearListView(),await this.renderDetailView(l)):(this.dom.filterContainer.style.display="block",await this.renderListView(),await this.updateTableView())}renderDashboardView(){let t=this.t("dashboard");document.title=`${this.applicationTitle}`,this.dom.title.textContent=t,document.querySelectorAll("#entity-menu a").forEach(t=>t.classList.remove("active")),this.dom.filterContainer.style.display="none",this.clearListView(),this.dom.tableDataContainer.innerHTML=`<p>${this.t("welcome_dashboard")}</p>`}async renderListView(){this.clearListView(),this.dom.title.textContent=this.t("list_of",this.currentEntity.displayName),await this.renderFilters()}async updateTableView(){let t=this.getFieldsForQuery(this.currentEntity,1),e=(this.state.page-1)*this.state.limit,i=[];this.currentEntity.filters&&Object.entries(this.state.filters).forEach(([t,e])=>{let a=this.currentEntity.filters.find(e=>e.name===t);e&&a&&i.push({field:t,value:e,operator:a.operator||"EQUALS"})});let a=[];this.state.orderBy&&this.state.orderBy.field&&a.push(this.state.orderBy);let n=`
        ...
        `;try{this.dom.tableDataContainer.innerHTML=this.t("loading"),this.dom.paginationContainer.innerHTML="";let s=await this.gqlQuery(n,{limit:this.state.limit,offset:e,orderBy:a,filter:i}),l=s[this.currentEntity.pluralName];this.renderTable(l.items),this.renderPagination(l)}catch(r){this.dom.tableDataContainer.innerHTML=`<p style="color: red;">${r.message}</p>`}}renderTable(t){let e=Object.keys(this.currentEntity.columns),i=`
        ...
        `;try{let n=await this.gqlQuery(a,{id:t}),s=n[this.currentEntity.name],l=`<div class="back-controls">
                ...
                <div class="table-container detail-view">
                    <table class="table">
                        <tbody>`;for(let r in this.currentEntity.columns){let o=this.currentEntity.columns[r],d=o.references;if(l+=`<tr>
                                    ...
                                    <td>`,o.isForeignKey&&s[d]){let h=s[d][this.currentEntity.displayField];null===h&&(h="N/A"),l+=`<span>${h}</span>`}else if((o.type.includes("boolean")||r===this.currentEntity.activeField)&&this.config.booleanDisplay){let c=1===s[r]||"1"===s[r]||!0===s[r];l+=`<span>${c?this.t(this.config.booleanDisplay.trueLabelKey):this.t(this.config.booleanDisplay.falseLabelKey)}</span>`}else l+=`<span>${null!==s[r]?s[r]:"N/A"}</span>`;l+="</td></tr>"}l+="</tbody></table></div>",this.dom.tableDataContainer.innerHTML=l,document.getElementById("back-to-list").onclick=()=>history.back()}catch(u){this.dom.tableDataContainer.innerHTML=`<p style="color: red;">${this.t("failed_to_fetch_details")}</p>`}}async renderForm(t=null){this.dom.modalTitle.textContent=t?this.t("edit_entity",this.currentEntity.displayName):this.t("add_new_entity",this.currentEntity.displayName),this.dom.form.innerHTML=this.t("loading"),this.openModal();let e=this._invokeRenderHook(t?"update":"insert",{entity:this.currentEntity,container:this.dom.form,id:t});if(e)return;let i={};if(t){let a=this.getFieldsForQuery(this.currentEntity,2),n=`query GetForEdit($id: String!) { ${this.currentEntity.name}(id: $id) { ${a} } }`,s=await this.gqlQuery(n,{id:t});i=s[this.currentEntity.name]}let l="";for(let r in this.currentEntity.columns){if(r===this.currentEntity.primaryKey)continue;let o=this.currentEntity.columns[r],d="";if(o.isForeignKey&&i[o.references]?d=i[o.references][this.config.entities[this.camelCase(o.references)].primaryKey]:void 0!==i[r]&&(d=i[r]),l+='<div class="form-group">',l+=`<label for="${r}">${this.getEntityLabel(this.currentEntity,r)}</label>`,o.isForeignKey){let h=o.references,c=this.config.entities[this.camelCase(h)],u=c?await this.fetchAll(c,{activeOnly:!t}):[],g=c.displayField||"name";l+=`<select id="${r}" name="${r}">`,l+=`<option value="">${this.t("select_option")}</option>`,u.forEach(t=>{let e=t[c.primaryKey],i=t[g];l+=`<option value="${e}" ${e==d?"selected":""}>${i}</option>`}),l+="</select>"}else{let m=this.currentEntity.activeField||this.defaultActiveField,y="text";o.type.includes("int")&&(y="number"),(o.type.includes("boolean")||r===m)&&(y="checkbox"),o.dataType.includes("datetime")||o.dataType.includes("timestamp")?y="datetime-local":o.dataType.includes("date")?y="date":o.dataType.includes("time")?y="time":o.dataType.includes("float")&&(y='number" step="any'),"checkbox"===y?l+=`<input type="checkbox" id="${r}" name="${r}" ${d?"checked":""}>`:l+=`<input type="${y}" id="${r}" name="${r}" value="${d}">`}l+="</div>"}this.dom.form.innerHTML=l,this.dom.form.closest(".modal").querySelector(".modal-footer").innerHTML=`
        <button type="submit" class="btn btn-primary">${this.t("save")}</button> 
        <button type="button" class="btn btn-secondary" data-dismiss="modal">${this.t("close")}</button> `,this.dom.form.closest(".modal").querySelector(".modal-footer").querySelector('[type="submit"]').addEventListener("click",()=>this.handleSave(t)),this.dom.form.onsubmit=e=>{e.preventDefault(),this.handleSave(t)}}async handleSave(t){let e=this._invokeRenderHook(`save_${t?"update":"insert"}`,{entity:this.currentEntity,form:this.dom.form,id:t,app:this});if(e)return;this.currentEntity.activeField||this.defaultActiveField;let i=new FormData(this.dom.form),a={};for(let n in this.currentEntity.columns){if(n===this.currentEntity.primaryKey)continue;let s=this.currentEntity.columns[n];if(i.has(n)){let l=i.get(n);"checkbox"===this.dom.form.querySelector(`[name="${n}"]`).type&&(l=this.dom.form.querySelector(`[name="${n}"]`).checked),s.type.includes("boolean")||s.type.includes("int")?""===(l=!0===l?1:!1===l?0:l)||isNaN(l)||(l=Number(l)):s.type.includes("float")&&(l=l?Number(l):null),a[n]=l}}for(let r in this.currentEntity.columns){if(r===this.currentEntity.primaryKey)continue;let o=this.currentEntity.columns[r];"checkbox"===this.dom.form.querySelector(`[name="${r}"]`).type&&(this.dom.form.querySelector(`[name="${r}"]`).checked||(o.type.includes("boolean")?a[r]=!1:a[r]=0))}let d=this.ucFirst(this.currentEntity.name),h=t?`update${d}`:`create${d}`,c=this.getFieldsForQuery(this.currentEntity,1,!0),u=t=>{if("string"==typeof t){let e=t.replace(/\\/g,"\\\\").replace(/"/g,'\\"');return`"${e}"`}return null==t?"null":t},g=Object.entries(a).map(([t,e])=>`${t}: ${u(e)}`).join(", "),m=`input: {${g}}`;t&&(m=`id: "${t}", ${m}`);let y=`
            mutation {
                ${h}(${m}) {
                    ${c}
                }
