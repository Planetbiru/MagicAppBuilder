...
<td>`,d.isForeignKey&&l[h]){let c=l[h][this.currentEntity.displayField];null===c&&(c="N/A"),r+=`<span>${c}</span>`}else if((d.type.includes("boolean")||o===this.currentEntity.activeField)&&this.config.booleanDisplay){let u=1===l[o]||"1"===l[o]||!0===l[o];r+=`<span>${u?this.t(this.config.booleanDisplay.trueLabelKey):this.t(this.config.booleanDisplay.falseLabelKey)}</span>`}else r+=`<span>${null!==l[o]?l[o]:"N/A"}</span>`;r+="</td></tr>"}r+="</tbody></table></div>",this.dom.tableDataContainer.innerHTML=r,document.getElementById("back-to-list").onclick=()=>{if(history.length>1&&document.referrer&&new URL(document.referrer).origin===window.location.origin){history.back();return}let t=new URLSearchParams(window.location.hash.split("?")[1]),e=t.get("from");e?window.location.hash=decodeURIComponent(e):this.navigateTo(this.currentEntity.name)}}catch(m){this.dom.tableDataContainer.innerHTML=`<p style="color: red;">${this.t("failed_to_fetch_details")}</p>`}}async renderForm(t=null){this.dom.modalTitle.textContent=t?this.t("edit_entity",this.currentEntityDisplayName):this.t("add_new_entity",this.currentEntityDisplayName),this.dom.form.innerHTML=this.t("loading"),this.openModal();let e=t?"update":"insert",i=this._invokeRenderHook(e,{entity:this.currentEntity,container:this.dom.form,id:t});if(i)return;let a={};if(t){let n=this.getFieldsForQuery(this.currentEntity,2,2),s=`query GetForEdit($id: String!) { ${this.currentEntity.name}(id: $id) { ${n} } }`,l=await this.gqlQuery(s,{id:t}),r=l.data;a=r[this.currentEntity.name]}let o="";for(let d in this.currentEntity.columns){if(d===this.currentEntity.primaryKey&&("insert"===e&&"autogenerated"===this.currentEntity.columns[d].primaryKeyValue||"update"===e&&"manual-all"!=this.currentEntity.columns[d].primaryKeyValue)||this.currentEntity.backendHandledColumns&&this.currentEntity.backendHandledColumns.includes(d))continue;let h=this.currentEntity.columns[d],c="";h.isForeignKey&&a[h.references]?c=a[h.references][this.config.entities[this.camelCase(h.references)].primaryKey]:void 0!==a[d]&&(c=a[d]);let u="",m="text";if(u+=`<label for="${d}">${this.getEntityLabel(this.currentEntity,d)}</label>`,h.isForeignKey){let g=h.references,y=this.config.entities[this.camelCase(g)],p=y?await this.fetchAll(y,{activeOnly:!t}):[],f=y.displayField||y.primaryKey;u+=`<select id="${d}" name="${d}">`,u+=`<option value="">${this.t("select_option")}</option>`,p.forEach(t=>{let e=t[y.primaryKey],i=t[f]||e;u+=`<option value="${e}" ${e==c?"selected":""}>${i}</option>`}),u+="</select>",m="select"}else if("textarea"===h.element)u+=`<textarea id="${d}" name="${d}" spellcheck="false" autocomplete="off">${c}</textarea>`,m="textarea";else{let b=this.currentEntity.activeField||this.defaultActiveField,E="text";h.type.includes("int")&&(E="number"),(h.type.includes("boolean")||d===b)&&(E="checkbox"),h.dataType.includes("datetime")||h.dataType.includes("timestamp")?(E="datetime-local",m="text"):h.dataType.includes("date")?(E="date",m="text"):h.dataType.includes("time")?(E="time",m="text"):h.dataType.includes("float")&&(E='number" step="any',m="text"),"checkbox"===E?(u+=`<input type="checkbox" id="${d}" name="${d}" ${c?"checked":""}>`,m="checkbox"):(u+=`<input type="${E}" id="${d}" name="${d}" value="${c}" autocomplete="off">`,m="text")}o+=`<div class="form-group form-element-${m}">${u}</div>`}o+='<div id="form-error"></div>',this.dom.form.innerHTML=o,this.dom.form.closest(".modal").querySelector(".modal-footer").innerHTML=`
