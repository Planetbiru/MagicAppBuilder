...
`;try{this.dom.tableDataContainer.innerHTML=this.t("loading"),this.dom.paginationContainer.innerHTML="";let l=await this.gqlQuery(n,{limit:this.state.limit,offset:e,orderBy:a,filter:i}),s={};if(l&&l.data){let r=l&&l.data?l.data:{};s=r&&r[this.currentEntity.pluralName]?r[this.currentEntity.pluralName]:{}}s&&s.items&&s.items.length>0?this.renderTable(s.items):this.dom.tableDataContainer.innerHTML='<p style="color: red;">Data not found</p>',this.renderPagination(s),s.errors&&s.errors.length>0&&console.error(s.errors)}catch(o){this.dom.tableDataContainer.innerHTML=`<p style="color: red;">${o.message}</p>`}}renderTable(t){let e=this.currentEntity.listColumns||[],i=Object.keys(this.currentEntity.columns).filter(t=>0===e.length||e.includes(t)),a=`
